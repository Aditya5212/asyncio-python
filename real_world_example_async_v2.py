...
--------------------------------
Async version v2:
- Improvements over async_v1:
    - Use an async HTTP client (`httpx.AsyncClient`) and async file I/O
        (`aiofiles`) for non-blocking downloads without threads.
    - Offload CPU-bound image processing to a `ProcessPoolExecutor` to gain
        true parallelism for CPU-heavy work (avoiding GIL limits).

What this file demonstrates:
- How to perform fully asynchronous downloads using `httpx` and `aiofiles`.
- How to combine async downloads with offloading CPU work to a process pool.

Notes:
- This version reduces the overhead of threads for downloads and uses native
    async I/O for network + disk writes. Processing still runs in separate
...
async def download_single_image(
    ...
    # This function uses an async HTTP client (httpx) and async file I/O
    # (aiofiles) so it does not block the event loop while downloading.
...
async def download_images(urls: list) -> list[Path]:
    # Using an async client + TaskGroup to schedule downloads concurrently.
    # No threads are required for network I/O here because httpx + aiofiles
    # perform non-blocking I/O that cooperates with the event loop.
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    # Offload the CPU-bound processing to a ProcessPoolExecutor to get true
    # parallelism (bypassing the GIL). `run_in_executor` returns awaitables
    # that complete when the worker process finishes.
