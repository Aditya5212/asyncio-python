...
--------------------------------
Async version v1 (tutorial progression):
- Goal: Convert the synchronous baseline to an async-friendly workflow while
    minimizing invasive changes.

What we changed compared to sync_v1:
- Downloads: still use the blocking `requests` download function, but we
    offload it to threads using `asyncio.to_thread` so the event loop remains
    responsive and multiple downloads can run concurrently.
- Processing: likewise, the CPU-bound `process_single_image` is offloaded to
    background threads via `asyncio.to_thread` inside `process_images`.
- Orchestration: we use `asyncio.TaskGroup` to schedule and wait for groups
    of tasks in a clean, structured way.

Why this version exists:
- It is the minimal async conversion showing how `to_thread` and TaskGroup
    can be used to get concurrency without fully rewriting blocking code.

Note: this tutorial intentionally does not document `process_single_image`
internals since the focus is on asyncio patterns and scheduling.
"""
...
def download_single_image(url: str, img_num: int) -> Path:
    # Blocking download helper kept as-is (uses requests). We'll call this
    # function inside `asyncio.to_thread` so downloads run in background
    # threads and do not block the event loop.
...
async def download_images(urls: list) -> list[Path]:
    # Schedule downloads concurrently by offloading the blocking
    # `download_single_image` function to background threads via
    # `asyncio.to_thread` and grouping them with `TaskGroup`.
    # TaskGroup ensures structured concurrency and waits for all tasks to
    ...
    # TaskGroup tasks are complete here; extract their results.
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    # Offload CPU-bound image processing to threads using `to_thread`.
    # For true CPU parallelism a ProcessPool would be better, but this shows
    # how to convert blocking work to background workers without changing the
    # processing function.
    async with asyncio.TaskGroup() as tg:
        tasks = [
...
async def main():
    ...
    # Downloads are scheduled concurrently and run in background threads.
    img_paths = await download_images(IMAGE_URLS)

    proc_start_time = time.perf_counter()

    # Processing is scheduled concurrently in background threads (via to_thread)
