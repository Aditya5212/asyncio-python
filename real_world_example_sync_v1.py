...
--------------------------------
Synchronous baseline (v1):
- Downloads images sequentially using `requests`.
- Processes images sequentially in the same thread.

Purpose:
- Demonstrates the blocking/synchronous approach so we can compare
    timings and behavior with the async versions.

Improvements to make in async versions (summary):
- async_v1: offload blocking I/O and CPU-bound work so downloads/processes
    can overlap (use threads/processes and asyncio helpers).
- async_v2: use fully async HTTP client and async file I/O to avoid threads
    for downloads, but still offload CPU-bound processing to a ProcessPool.
- async_v3: add concurrency limits (Semaphore) for downloads and tune
    process pool size for CPU-bound work.

Note: this file intentionally keeps the processing implementation as-is; the
`process_single_image` details are not re-explained here (this is an asyncio
tutorial focused on scheduling and event loops).
"""
...
def download_single_image(session: requests.Session, url: str, img_num: int) -> Path:
    # This function runs synchronously and blocks until the download
    # completes. When called repeatedly from the main thread the downloads
...
def download_images(urls: list) -> list[Path]:
    # Sequential download loop using blocking requests.Session. Each call to
    # download_single_image blocks the thread until the HTTP response is
...
def main():
    ...
    # Downloads happen serially here.
    img_paths = download_images(IMAGE_URLS)

    proc_start_time = time.perf_counter()

    # Processing also happens serially in the main thread.
